...
export default class Home extends Component {
	...
	async loadBlockchain() {
        ...
        this.setState({ networkId: networkId, web3: web3, contractInstance: instance });
    ...
    async displayForSale() {
        ...
        if(this.state.products.length == 0) {
            productsHtml = (
                <div key="0" className="center">Nothing listed yet...</div>
            )
        } else {
	...
	async getLatestNFT(amount) {
	    ...
	    const nftCount = parseInt(await this.state.contractInstance.methods.getProductsLength().call())
	    ...
	    for(let i = nftCount; i > condition; i--) {
	        let product = await this.state.contractInstance.methods.products(i - 1).call()
	        product = {
